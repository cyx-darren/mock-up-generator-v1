...
export function AnimatedPullToRefresh({
  ...
  const [pullDistance, setPullDistance] = useState(0);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [refreshStage, setRefreshStage] = useState<'idle' | 'pulling' | 'ready' | 'refreshing'>('idle');
  ...
  return (
    <div 
      ...
      style={{ 
        transform: `translateY(${pullDistance}px)`,
        transition: refreshStage === 'idle' || isRefreshing ? 'transform 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none'
      }}
      onTouchStart={handleTouchStart}
      onTouchMove={handleTouchMove}
      onTouchEnd={handleTouchEnd}
