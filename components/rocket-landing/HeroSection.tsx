...
import Button from '@/components/rocket-ui/Button';
...
const HeroSection: React.FC<HeroSectionProps> = ({ onStartCreating, onBrowseTemplates }) => {
  const [currentMockup, setCurrentMockup] = useState<number>(0);
  const [mockupsGenerated, setMockupsGenerated] = useState<number>(47832);

  const mockupFrames: MockupFrame[] = [
    {
      device: 'Custom T-Shirt Mockup',
      image: '/assets/shirt-hero.png',
      color: '#1d1d1f'
    },
    {
      device: 'Custom Mug Mockup',
      image: '/assets/ceramic-mug-hero.png',
      color: '#f5f5f7'
    },
    {
      device: 'Custom Tote Bag Mockup',
      image: '/assets/cotton-bag-hero.png',
      color: '#2c2c2e'
    }
  ];
              ...
              <div className="relative h-80 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl overflow-hidden mb-6">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="relative transform transition-all duration-1000 ease-in-out">
                    {mockupFrames[currentMockup]?.device === 'Custom T-Shirt Mockup' && (
                      <div className="w-64 h-64 bg-gray-100 rounded-lg p-4 shadow-xl">
                    ...
                    {mockupFrames[currentMockup]?.device === 'Custom Mug Mockup' && (
                      <div className="w-64 h-64 bg-gray-100 rounded-lg p-4 shadow-xl">
                        <div className="w-full h-full bg-white rounded-lg overflow-hidden relative">
                          <Image
                            src={mockupFrames[currentMockup]?.image}
                            alt="Custom Mug mockup"
                            fill
                            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                            className="object-cover"
                            unoptimized
                    ...
                    {mockupFrames[currentMockup]?.device === 'Custom Tote Bag Mockup' && (
                      <div className="w-64 h-64 bg-gray-100 rounded-lg p-4 shadow-xl">
                        <div className="w-full h-full bg-white rounded-lg overflow-hidden relative">
                          <Image
                            src={mockupFrames[currentMockup]?.image}
                            alt="Custom Tote Bag mockup"
                            fill
                            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                            className="object-cover"
                            unoptimized
