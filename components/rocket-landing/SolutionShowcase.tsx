...
import { Sparkles, Upload, Plus, Smartphone, Zap, RotateCcw, Image as ImageIcon, Check, Download, Share, Palette } from 'lucide-react';
...
const SolutionShowcase: React.FC = () => {
  ...
  const renderDevicePreview = (imageUrl: string, isResult: boolean = false) => {
    const containerClasses = isResult ? 'shadow-2xl' : 'shadow-xl';

    switch (selectedDevice) {
      case 'iphone':
        return (
          <div className={`w-48 h-72 bg-slate-800 rounded-3xl p-2 ${containerClasses}`}>
            <div className="w-full h-full bg-white rounded-2xl overflow-hidden relative">
              <Image
                src={imageUrl}
                alt={isResult ? 'Generated mockup' : 'Your design'}
                fill
      ...
      case 'macbook':
        return (
          <div className={`w-64 h-40 bg-slate-300 rounded-lg p-2 ${containerClasses}`}>
      ...
      case 'ipad':
        return (
          <div className={`w-56 h-72 bg-slate-700 rounded-2xl p-3 ${containerClasses}`}>
      ...
      case 'watch':
        return (
          <div className={`w-32 h-40 bg-slate-800 rounded-3xl p-2 ${containerClasses}`}>
            <div className="w-full h-full bg-white rounded-2xl overflow-hidden relative">
              <Image
                src={imageUrl}
                alt={isResult ? 'Generated mockup' : 'Your design'}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {sampleImages?.map((sample, index) => (
                        ...
                        >
                          <div className="aspect-square rounded-lg overflow-hidden border-2 border-transparent group-hover:border-blue-400 transition-colors relative">
                            <Image
                              src={sample?.url}
                              alt={sample?.name}
                              fill
                ...
                <div className="space-y-3">
                  ...
                  {(uploadedImage || showResult) && (
                    <button
                      ...
                      className="w-full flex items-center justify-center px-4 py-3 rounded-lg font-medium transition-all border border-slate-300 text-slate-700 hover:bg-slate-50"
                    >
            ...
            <div className="bg-white rounded-xl p-8 shadow-lg border border-slate-200">
              ...
              <div className="relative h-96 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl overflow-hidden flex items-center justify-center">
                ...
                {uploadedImage && !isProcessing && !showResult && (
                  <div className="text-center">
                    <div className="relative inline-block">
                      {renderDevicePreview(uploadedImage)}
                    </div>
                ...
                {showResult && uploadedImage && (
                  <div className="text-center">
                    <div className="relative inline-block">
                      {renderDevicePreview(uploadedImage, true)}
                    ...
                    <div className="mt-4 space-y-2">
                      ...
                      <div className="flex justify-center space-x-2">
                        ...
                        <button className="flex items-center px-3 py-2 border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
