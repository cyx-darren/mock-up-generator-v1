...
export async function createGiftItem(
  itemData: Database['public']['Tables']['gift_items']['Insert']
) {
  const { data, error } = await (supabaseAdmin as any)
    .from('gift_items')
    .insert([itemData])
...
export async function getGiftItems(filters?: {
  ...
  let query = (supabaseAdmin as any).from('gift_items').select(`
      *,
...
export async function updatePlacementConstraint(id: string, updates: Partial<PlacementConstraint>) {
  const { data, error } = await (supabaseAdmin as any)
    .from('placement_constraints')
    .update(updates)
    .eq('id', id)
...
export async function getAuditLogs(filters?: {
  ...
  let query = (supabaseAdmin as any).from('audit_log').select(`
      *,
...
export async function getDashboardStats() {
  const [itemsResult, sessionsResult, constraintsResult] = await Promise.all([
    (supabaseAdmin as any).from('gift_items').select('status').eq('is_active', true),
    ...
    (supabaseAdmin as any).from('placement_constraints').select('is_validated'),
  ]);
  ...
  return {
    ...
    sessions: {
      ...
      completed: sessions.filter((session: any) => session.status === 'completed').length,
      processing: sessions.filter((session: any) => session.status === 'processing').length,
      failed: sessions.filter((session: any) => session.status === 'failed').length,
    },
    constraints: {
      total: constraints.length,
      validated: constraints.filter((constraint: any) => constraint.is_validated).length,
      pending: constraints.filter((constraint: any) => !constraint.is_validated).length,
    },
  };
}
