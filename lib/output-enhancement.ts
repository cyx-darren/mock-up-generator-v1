...
export class OutputEnhancer {
  ...
  private applySharpening(
    ...
    for (let i = 0; i < imageData.data.length; i += 4) {
      const original = [imageData.data[i], imageData.data[i + 1], imageData.data[i + 2]];

      const blurred = [blurredData.data[i], blurredData.data[i + 1], blurredData.data[i + 2]];

      // Calculate difference
      const diff = [original[0] - blurred[0], original[1] - blurred[1], original[2] - blurred[2]];
  ...
  private applyColorCorrection(
    ...
    const { saturation = 1.0, vibrance = 0, temperature = 0, tint = 0 } = options!;
  ...
  private applyContrastAdjustment(
    ...
    const { amount = 1.0, highlights = 0, shadows = 0, midtones = 1.0 } = options!;
  ...
  private applyEdgeEnhancement(
    ...
    for (let i = 0; i < imageData.data.length; i += 4) {
      ...
      result.data[i] = Math.max(
        0,
        Math.min(255, imageData.data[i] + imageData.data[i] * finalStrength)
      );
      result.data[i + 1] = Math.max(
        0,
        Math.min(255, imageData.data[i + 1] + imageData.data[i + 1] * finalStrength)
      );
      result.data[i + 2] = Math.max(
        0,
        Math.min(255, imageData.data[i + 2] + imageData.data[i + 2] * finalStrength)
      );
      result.data[i + 3] = imageData.data[i + 3];
    }
