async function verifyConstraints() {
  ...
  try {
    ...
    products.forEach((product, index) => {
      ...
      console.log(`${String(index + 1).padStart(2)}. ${product.name.padEnd(25)} | H:${horizontal} V:${vertical} A:${allOver}`);
    });
    ...
    products.forEach(product => {
      const optionCount = (product.horizontal_enabled ? 1 : 0) + 
    ...
    console.log(`Products with ≥2 constraint options: ${productsWithAtLeast2Options}/${products.length}`);
    
    if (productsWithAtLeast2Options === products.length) {
      console.log(`✅ SUCCESS: All products have at least 2 constraint options!`);
      console.log(`✅ Task 3.1.2 verification requirement met: "Each product has at least 2 constraint options"`);
    } else {
      console.log(`❌ INCOMPLETE: ${products.length - productsWithAtLeast2Options} products need more constraint options`);
    }
