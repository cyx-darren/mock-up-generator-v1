async function verifyProducts() {
  ...
  try {
    ...
    console.log(`\n=== SUMMARY ===`);
    console.log(
      `✓ Products uploaded: ${products.length}/20 ${products.length >= 20 ? '(COMPLETE)' : '(INCOMPLETE)'}`
    );
    console.log(
      `✓ Products with thumbnails: ${productsWithThumbnails}/${products.length} ${productsWithThumbnails === products.length ? '(ALL)' : '(SOME MISSING)'}`
    );
    console.log(
      `✓ Categorized products: ${categorizedProducts}/${products.length} ${categorizedProducts === products.length ? '(ALL)' : '(SOME MISSING)'}`
    );
    console.log(`✓ Categories used: ${Array.from(categories).join(', ')}`);

    // Check if task 3.1.1 requirements are met
    const task311Complete =
      products.length >= 20 &&
