...
const testCases = [
  ...
  },
  {
    ...
    height: 200,
    greenPixels: [
      { x: 100, y: 100 }, { x: 101, y: 100 }, { x: 100, y: 101 }, { x: 101, y: 101 }
    ],
    expectedArea: 4,
...
async function runConstraintValidationTests() {
  ...
  for (const testCase of testCases) {
    ...
    console.log(`   Image: ${testCase.width}x${testCase.height}, Expected: ${testCase.expectedArea} pixels (${testCase.expectedPercentage}%)`);
    ...
    try {
      ...
      const percentageMatch = Math.abs(detectedArea.percentage - testCase.expectedPercentage) <= percentageTolerance;
      
      console.log(`   ✓ Detected: ${detectedArea.pixels} pixels (${detectedArea.percentage}%)`);
      console.log(`   ✓ Bounds: ${detectedArea.bounds.width}x${detectedArea.bounds.height} at (${detectedArea.bounds.x}, ${detectedArea.bounds.y})`);
      console.log(`   ✓ Quality: ${detectedArea.quality}, Aspect Ratio: ${detectedArea.aspectRatio}`);
      ...
      console.log(`   ✓ Usable Area: ${validation.usableArea.pixels} pixels (${validation.usableArea.percentage}%)`);
      ...
      console.log(`   ✓ Center Offset: (${metrics.centerOffset.x.toFixed(1)}, ${metrics.centerOffset.y.toFixed(1)})`);
      console.log(`   ✓ Edge Distances: T:${metrics.edgeDistances.top} R:${metrics.edgeDistances.right} B:${metrics.edgeDistances.bottom} L:${metrics.edgeDistances.left}`);
      console.log(`   ✓ Compactness: ${metrics.compactness.toFixed(3)}, Fragments: ${metrics.fragmentCount}`);
      ...
      if (pixelMatch && percentageMatch && detectedArea.quality >= 0) {
        ...
        console.log(`   ❌ FAIL - Detection outside tolerance (pixels: ${pixelMatch}, %: ${percentageMatch})\n`);
      }
  ...
  if (passedTests === totalTests) {
    ...
    console.log(`❌ ${totalTests - passedTests} tests failed. Review constraint detection algorithms.`);
    return false;
  }
}
