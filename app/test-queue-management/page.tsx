...
import { getQueueManager, QueueManager, QueueJob, QueueMetrics, JobPriority, JobStatus } from '../../lib/queue-management';

export default function TestQueueManagementPage() {
  const [queueManager] = useState(() => getQueueManager({
    maxConcurrentJobs: 3,
    maxQueueSize: 50,
  ...
  const [jobPayload, setJobPayload] = useState('{"productId": "test-product", "logoUrl": "test-logo.png"}');

  const refreshData = useCallback(() => {
    const allJobs = queueManager.getJobs();
    setJobs(allJobs.sort((a, b) => b.metadata.submittedAt.getTime() - a.metadata.submittedAt.getTime()));
  ...
  useEffect(() => {
    ...
    queueManager.registerProcessor('mockup_generation', async (job: QueueJob) => {
      ...
      job.progress = { current: 25, total: 100, stage: 'preparing_input', message: 'Preparing input data', percentage: 25 };
      
      await new Promise(resolve => setTimeout(resolve, 1500));
      job.progress = { current: 50, total: 100, stage: 'ai_processing', message: 'Processing with AI', percentage: 50 };
      
      await new Promise(resolve => setTimeout(resolve, 2000));
      job.progress = { current: 75, total: 100, stage: 'finalizing', message: 'Finalizing mockup', percentage: 75 };
              ...
              <div className="bg-white p-6 rounded-lg shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-bold text-yellow-600">{metrics.processingJobs}</div>
                  </div>
                  <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    {selectedJob.metadata.processingTime && (
                      <div>Processing Time: {formatDuration(selectedJob.metadata.processingTime)}</div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    <div>Retries: {selectedJob.metadata.retryCount}/{selectedJob.metadata.maxRetries}</div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="text-sm space-y-2">
                <div>• <strong>Queue Algorithm:</strong> Priority-based with FIFO within priority levels</div>
                <div>• <strong>Concurrency:</strong> Configurable concurrent job processing</div>
                <div>• <strong>Job Types:</strong> Support for multiple job processor types</div>
                <div>• <strong>Monitoring:</strong> Real-time metrics and performance tracking</div>
                <div>• <strong>Persistence:</strong> In-memory with configurable persistence options</div>
                <div>• <strong>Error Recovery:</strong> Automatic retry with configurable limits</div>
                <div>• <strong>Status Callbacks:</strong> Real-time job status notifications</div>
                <div>• <strong>Progress Tracking:</strong> Detailed progress updates during processing</div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 p-4 bg-green-50 rounded-lg">
            ...
            <div className="text-sm text-green-700">
              ✅ Process 10 jobs concurrently - Use "Add 10 Random Jobs" button to test<br/>
              ✅ Priority handling - Different priorities are processed in order<br/>
              ✅ Queue monitoring - Real-time metrics and job status tracking<br/>
              ✅ Job cancellation - Cancel button available for queued/processing jobs<br/>
              ✅ Status tracking - Complete job lifecycle from pending to completed/failed<br/>
              ✅ Queue persistence - Jobs maintained in queue with full state management
            </div>
          </div>
        </div>
      </div>
