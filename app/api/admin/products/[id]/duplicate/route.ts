...
export const POST = withPermission('canCreateProducts', async (request: NextRequest, user) => {
  try {
    const supabase = createClient();
    // Extract id from URL pathname
    const url = new URL(request.url);
    const pathSegments = url.pathname.split('/');
    const id = pathSegments[pathSegments.length - 2]; // Get the id from /api/admin/products/[id]/duplicate
    ...
    const duplicateProduct = {
      ...
      base_image_url:
        originalProduct.base_image_url ||
