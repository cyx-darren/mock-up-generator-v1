...
export default function TestQualityValidation() {
  ...
  const validateImage = async () => {
    ...
    try {
      const result = await validator.validateQuality(selectedImage.src, customRules, {
        enableAutoRegenerate: autoRegenerate,
        strictMode: validationRules === 'strict',
      });
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full mx-auto mb-1 ${
                          img.expectedQuality === 'high'
                            ? 'bg-green-500'
                            : img.expectedQuality === 'medium'
                              ? 'bg-yellow-500'
                              : 'bg-red-500'
                        }`}
                      ></div>
              ...
              <div className="space-y-4">
                ...
                {customRules && (
                  <div className="text-sm text-gray-600 space-y-1">
                    ...
                    <div>
                      Min Resolution: {customRules.technical.minimumResolution.width}×
                      {customRules.technical.minimumResolution.height}
                    </div>
                  </div>
                )}
              </div>
                      ...
                      <p>
                        ...
                        <span
                          className={`font-medium ${
                            selectedImage.expectedQuality === 'high'
                              ? 'text-green-600'
                              : selectedImage.expectedQuality === 'medium'
                                ? 'text-yellow-600'
                                : 'text-red-600'
                          }`}
                        >
                          {selectedImage.expectedQuality}
                        </span>
                      </p>
                    </div>
                  )}
                ...
                <div>
                  ...
                  {validationResult ? (
                    <div className="space-y-3">
                      <div
                        className={`text-center p-4 rounded-lg ${getGradeColor(validationResult.metrics.overall.grade)}`}
                      >
                        <div className="text-3xl font-bold">
                          {validationResult.metrics.overall.grade}
                        </div>
                        <div className="text-sm">
                          Score: {validationResult.metrics.overall.score.toFixed(1)}/100
                        </div>
                        <div className="text-xs">
                          Confidence:{' '}
                          {(validationResult.metrics.overall.confidence * 100).toFixed(1)}%
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-lg font-bold text-blue-600">
                        {validationResult.metrics.sharpness.score.toFixed(0)}
                      </div>
                      <div className="text-xs text-gray-600">Sharpness</div>
                      <div className="text-xs text-gray-500">
                        {validationResult.metrics.sharpness.focusQuality}
                      </div>
                    </div>

                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-lg font-bold text-green-600">
                        {validationResult.metrics.artifacts.score.toFixed(0)}
                      </div>
                      <div className="text-xs text-gray-600">Artifacts</div>
                      <div className="text-xs text-gray-500">
                        {validationResult.metrics.artifacts.severityLevel}
                      </div>
                    </div>

                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-lg font-bold text-purple-600">
                        {validationResult.metrics.colorAccuracy.score.toFixed(0)}
                      </div>
                      <div className="text-xs text-gray-600">Color</div>
                      <div className="text-xs text-gray-500">
                        {validationResult.metrics.colorAccuracy.whiteBalance.accuracy}
                      </div>
                    </div>

                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-lg font-bold text-orange-600">
                        {validationResult.metrics.placement.score.toFixed(0)}
                      </div>
                      <div className="text-xs text-gray-600">Placement</div>
                      <div className="text-xs text-gray-500">
                        {validationResult.metrics.placement.objectDetection.alignment.horizontal}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        <div>
                          Resolution: {validationResult.metrics.technical.resolution.width}×
                          {validationResult.metrics.technical.resolution.height}
                        </div>
                        <div>
                          Megapixels:{' '}
                          {validationResult.metrics.technical.resolution.megapixels.toFixed(1)}MP
                        </div>
                        <div>
                          Aspect: {validationResult.metrics.technical.aspectRatio.name || 'Custom'}
                        </div>
                        <div>
                          Alpha:{' '}
                          {validationResult.metrics.technical.metadata.hasAlpha ? 'Yes' : 'No'}
                        </div>
                        <div
                          className={
                            validationResult.metrics.technical.resolution.adequate
                              ? 'text-green-600'
                              : 'text-red-600'
                          }
                        >
                      ...
                      <div className="text-xs space-y-1">
                        {validationResult.metrics.sharpness.blurDetected && (
                          <div className="text-orange-600">
                            ⚠️ Blur detected ({validationResult.metrics.sharpness.blurType})
                          </div>
                        )}
                        {validationResult.metrics.colorAccuracy.colorCast.detected && (
                          <div className="text-yellow-600">
                            ⚠️ Color cast ({validationResult.metrics.colorAccuracy.colorCast.type})
                          </div>
                        )}
                        {validationResult.metrics.artifacts.artifactTypes.length > 0 && (
                          <div className="text-red-600">
                            ❌ Artifacts:{' '}
                            {validationResult.metrics.artifacts.artifactTypes.join(', ')}
                          </div>
                        )}
                    ...
                    {validationResult.failures.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-red-600 mb-2">
                          Quality Issues ({validationResult.failures.length})
                        </h4>
                        <div className="space-y-2">
                          {validationResult.failures.map((failure, index) => (
                            <div
                              key={index}
                              className={`p-3 rounded border text-sm ${getSeverityColor(failure.severity)}`}
                            >
            ...
            {batchResults.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  {batchResults.map((result, index) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-600">
                          {result.failures.length} issues, {result.recommendations.length}{' '}
                          recommendations
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <div
                          className={`px-2 py-1 rounded text-xs font-bold ${getGradeColor(result.metrics.overall.grade)}`}
                        >
                ...
                <div className="mt-4 text-sm text-gray-600">
                  ...
                  {(
                    (batchResults.filter((r) => r.passed).length / batchResults.length) *
                    100
                  ).toFixed(1)}
                  %)
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-gray-700">
                <div>
                  • <strong>Sharpness:</strong> Laplacian variance + Sobel edge detection
                </div>
                <div>
                  • <strong>Blur Types:</strong> Motion, Gaussian, and defocus classification
                </div>
                <div>
                  • <strong>Artifacts:</strong> JPEG compression, blocking, and ringing detection
                </div>
                <div>
                  • <strong>Color Cast:</strong> RGB deviation analysis with type identification
                </div>
                <div>
                  • <strong>Composition:</strong> Rule of thirds, symmetry, and balance scoring
                </div>
                <div>
                  • <strong>Resolution:</strong> Adequacy checking against minimum requirements
                </div>
                <div>
                  • <strong>Validation:</strong> Multi-criteria scoring with weighted averages
                </div>
                <div>
                  • <strong>Automation:</strong> Intelligent regeneration trigger system
                </div>
              </div>
            </div>
          </div>
