...
import { getFormatConverter, FormatOptions, ConversionResult, getFileSizeString, downloadConvertedImage } from '@/lib/format-conversion';
...
export default function TestFormatConversion() {
  ...
  const [recommendation, setRecommendation] = useState<{ format: 'png' | 'jpg' | 'webp'; reason: string; quality: number } | null>(null);
            ...
            {recommendation && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <div className="text-sm text-blue-700">
                  <div className="font-medium">{recommendation.format.toUpperCase()} at {recommendation.quality}% quality</div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {result && (
                    <div className="text-xs text-gray-600 mt-2">
                      <p>Format: {result.originalFormat} → {result.targetFormat.toUpperCase()}</p>
                      <p>Size: {getFileSizeString(result.originalSize.bytes)} → {getFileSizeString(result.convertedSize.bytes)}</p>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-xl font-bold text-blue-600">{result.targetFormat.toUpperCase()}</div>
                    <div className="text-sm text-gray-600">Format</div>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-xl font-bold text-green-600">{getFileSizeString(result.convertedSize.bytes)}</div>
                    <div className="text-sm text-gray-600">File Size</div>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-xl font-bold text-purple-600">{(result.compressionRatio * 100).toFixed(0)}%</div>
                    <div className="text-sm text-gray-600">Compression</div>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-xl font-bold text-orange-600">{result.conversionTime.toFixed(0)}ms</div>
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600 grid grid-cols-2 gap-2">
                    <div>Resolution: {result.convertedSize.width}×{result.convertedSize.height}</div>
            ...
            {batchResults.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="mt-4 text-sm text-gray-600">
                  Total conversions: {batchResults.length}, 
                  Average time: {(batchResults.reduce((sum, r) => sum + r.conversionTime, 0) / batchResults.length).toFixed(0)}ms
                </div>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-gray-700">
                <div>• <strong>PNG:</strong> Lossless compression with palette optimization</div>
                <div>• <strong>JPEG:</strong> DCT compression with quality control (1-100%)</div>
                <div>• <strong>WebP:</strong> Modern VP8/VP8L compression with alpha support</div>
                <div>• <strong>Detection:</strong> MIME type and extension-based format identification</div>
                <div>• <strong>Quality:</strong> Adaptive optimization based on image content analysis</div>
                <div>• <strong>Resize:</strong> High-quality resampling with multiple resize modes</div>
                <div>• <strong>Batch:</strong> Parallel processing with progress tracking</div>
                <div>• <strong>Analysis:</strong> Content-aware format recommendations</div>
              </div>
            </div>
          </div>
